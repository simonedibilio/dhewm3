...
void idGameLocal::Init( void ) {
	...
	msec = 16.0;     // 60fps
	gameMsec = msec; // shouldn't this be explicitly converted to int? or gameMsec be redefined as float? smells like something may be burning...
	gameFps = 60;    // 60fps
	...
	msec *= 0.96f; // hack: emulates OG D3 msec error in order to have exactly the same game logic speed (removed square...)
	gameMsec = msec;
...
void idGameLocal::SaveGame( idFile *f ) {
	...
	savegame.WriteFloat( preciseTime ); // breaks savegame compat
...
bool idGameLocal::InitFromSaveGame( const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld, idFile *saveGameFile ) {
	...
	savegame.ReadFloat(preciseTime); // breaks savegame compat
	...
	if ( gameSoundWorld ) {
		gameSoundWorld->SetSlowmoSpeed( slowmoMsec / (float)gameMsec ); // mh, is the float conversion necessary?
	}
#endif
...
void idGameLocal::ComputeSlowMsec() {
	...
	if ( slowmoState == SLOWMO_STATE_RAMPUP ) {
		...
		if ( fabs( delta ) < g_slowmoStepRate.GetFloat() ) {
			slowmoMsec = 4.0 * 60.0 / (float)gameFps; // shouldn't the float conversion be removed?
		...
		if ( fabs( delta ) < g_slowmoStepRate.GetFloat() ) {
			slowmoMsec = 16.0*60.0/(float)gameFps; // ... same as before?
			slowmoState = SLOWMO_STATE_OFF;
			if ( gameSoundWorld ) {
				gameSoundWorld->SetSlowmo( false );
			}
